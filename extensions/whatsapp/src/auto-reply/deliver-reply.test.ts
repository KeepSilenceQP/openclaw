...
function makeMsg(): NormalizedWebInboundMsg {
  return {
    event: {
      id: "msg-1",
    },
  ...
  } as unknown as NormalizedWebInboundMsg;
}
...
function mockFirstSendMediaFailure(msg: NormalizedWebInboundMsg, message: string) {
  (
    msg.platform.sendMedia as unknown as { mockRejectedValueOnce: (v: unknown) => void }
  ).mockRejectedValueOnce(new Error(message));
}

function mockFirstReplyFailure(msg: NormalizedWebInboundMsg, message: string) {
  (
    msg.platform.reply as unknown as { mockRejectedValueOnce: (v: unknown) => void }
  ).mockRejectedValueOnce(new Error(message));
}

function mockFirstReplyFailureWithWrappedError(msg: NormalizedWebInboundMsg, message: string) {
  (
    msg.platform.reply as unknown as { mockRejectedValueOnce: (v: unknown) => void }
...
function expectFirstSendMediaPayload(msg: NormalizedWebInboundMsg) {
...
function replyText(msg: NormalizedWebInboundMsg, callIndex = 0): string {
...
function mockSecondReplySuccess(msg: NormalizedWebInboundMsg) {
  (
